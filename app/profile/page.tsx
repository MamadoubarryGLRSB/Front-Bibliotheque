...
const mockUser = {
  ...
  borrowedBooks: [
    ...
    { title: "Brave New World", author: "Aldous Huxley", borrowedDate: "2024-02-20" },
    { title: "The Great Gatsby", author: "F. Scott Fitzgerald", borrowedDate: "2024-03-05" },
  ],
};
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    // Exemple d'appel API (décommentez et configurez votre backend)
    // fetch("/api/user")
    //   .then((res) => res.json())
  ...
  return (
    <div className="flex flex-col lg:flex-row lg:space-x-8 p-6 min-h-screen bg-gray-100">
      ...
      <aside className="lg:w-1/4 w-full bg-white p-4 rounded shadow-md mb-6 lg:mb-0">
        <h2 className="text-2xl font-bold mb-4 text-center">Informations Utilisateur</h2>
        <p className="mb-2">
      ...
      <section className="flex-1 bg-white p-6 rounded shadow-md">
        <h2 className="text-2xl font-bold mb-4 text-center">Historique des Livres Empruntés</h2>
          ...
          <table className="w-full border-collapse border border-gray-300">
            <thead>
              <tr className="bg-gray-200">
                ...
                <th className="border border-gray-300 px-4 py-2">Date d'emprunt</th>
                <th className="border border-gray-300 px-4 py-2">Date de retour</th>
              </tr>
            </thead>
            <tbody>
              {user.borrowedBooks.map((book: any, index: number) => (
                <tr key={index} className="text-center">
                  <td className="border border-gray-300 px-4 py-2">{book.title}</td>
                  <td className="border border-gray-300 px-4 py-2">{book.author}</td>
                  <td className="border border-gray-300 px-4 py-2">{book.borrowedDate}</td>
